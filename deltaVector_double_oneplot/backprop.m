function [net, adaptive_var, loss1, loss2] = backprop(data, net, parameter, adaptive_var, global_step)
    [Y, result] = forwordprop(net,data,parameter.dropout);
   ...
   %% 计算func的损失
    ...
    %% 训练模型
    %% [net, adaptive_var, gI] = train_deltaVector(net, parameter, adaptive_var, Y, g, global_step)
    for i=1:len-1
        [t_net, t_ada_var, ~] = train_deltaVector(net, parameter, adaptive_var, Y.clinVar{i}.Y, g_clinVar{i}, global_step);
        if i==1
            sum_net = t_net;
            sum_adv = t_ada_var;
        else
        ...
        [t_net, t_ada_var, ~] = train_deltaVector(net, parameter, adaptive_var, Y.func{i}.Y, g_func{i}, global_step);
        sum_net = net_add(sum_net,t_net);
        sum_adv = net_add(sum_adv,t_ada_var);
    end
    ...
    [sum_net1, sum_adv1, ~] = train_deltaVector(net, parameter, adaptive_var, Y.clinVar{len}.Y, g_clinVar{len}, global_step);
    [t_net, t_ada_var, ~] = train_deltaVector(net, parameter, adaptive_var, Y.func{len}.Y, g_func{len}, global_step);
