function [attentionLayer, adaptive_var, gI] = train_GAT(attentionLayer, parameter, adaptive_var, Y, g, global_step, G)
...
%     gQ = gpuArray(single(zeros(attentionLayer.key_size,m)));
    ...
    for i=1:heads
        %% 计算q,k,v的梯度
        gT = gB((i-1)*attentionLayer.size_per_head+1:i*attentionLayer.size_per_head,:);
        q = Y.q{i};
        k = Y.k{i};
        v = Y.v{i};
        A = Y.A{i};
